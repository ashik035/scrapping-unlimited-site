...
all_website_data = []
async def fetch_website_data(session, url):
    website_data = {}
    try:
...
async def scrape_websites(all_urls):
    async with aiohttp.ClientSession() as session:
        tasks = []
        for url in all_urls:
            tasks.append(fetch_website_data(session, url))

        website_data = await asyncio.gather(*tasks)
        return website_data

def url_match(url1, url2):
    url1_cleaned = re.sub(r'^https?://', '', url1)
    url2_cleaned = re.sub(r'^https?://', '', url2)
...
def csv_input_part ():
    ...
    try:
        number = int(input("How many websites do you want to scrape in one execution: "))
    except ValueError:
